...
const account1 = {
  ...
  movementsDates: [
    ...
    '2024-01-25T10:51:36.790Z', // yesterday
  ],
  currency: 'EUR',
  locale: 'pt-PT', // de-DE
};
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    labelDate.textContent = new Intl.DateTimeFormat(locale, timeDetails).format(
      now
    );
    // Clear input fields
    inputLoginUsername.value = inputLoginPin.value = '';

    // if there is already running timer it would be cleared
    if (timer) {
      clearInterval(timer);
    }
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    // if action is made then the user is active -> restore the session timer
    clearInterval(timer);
    timer = startLogOutTimer();
  }
});
